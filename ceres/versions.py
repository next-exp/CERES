...
def get_ceres_tag():
    ...
    ceres_tag = os.fsdecode(output)strip().split('\n')[0]
    return ceres_tag
...
def get_version():
    ...
    output  = check_output(cmd, shell=True, executable='/bin/bash')
    branch  = os.fsdecode(output)strip().split('\n')[0]
...
def get_ic_tag(templates):
    ...
    output = check_output(cmd, shell=True, executable='/bin/bash')
    ic_tag = os.fsdecode(output)strip().split('\n')[0]
    return ic_tag
...
def unstaged_changes():
    ...
    output = check_output(cmd, shell=True, executable='/bin/bash')
    files  = os.fsdecode(output)strip().split('\n')
    if files[0] != '':
